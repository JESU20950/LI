...
main:- N=11, retractall(bestRouteSoFar(_,_)),  assertz(bestRouteSoFar(100000,[])),  % "infinite" distance
       findall(I,between(2,N,I),Cities), calculate_distance, tsp( Cities, 0, [1] ).
...
calculate_distance:- between(2,11,City),  min(City, 11, S), assertz(distance(City,S)), write([City,S]).



min(_, 0 , 100000000).
min(City, City2,  S):- Cityaux is City2-1, min(City,Cityaux, Saux), distance(City, City2, R), R \= 0, R < Saux, !, S is R.
min(City,City2,S):-Cityaux is City2-1, min(City,Cityaux, S).
