...
writeClauses(MaxNumProf):-
    onelectureatmostonehourperday,
    twolecturesfromthesameyearnotoverlap,
    eachcourseexactlyoneteacher,
    eachcourseexactlyoneroom,
    eachcourseexactlynumHoursPerWeek,
    givenateacheranhourandadaycanotbeintwolectures,
    givenaroomanhouradaycannotbetwolectures,
    atmostMaxNumProf(MaxNumProf),
    
    relationcdhandcpwithcdhp,
    relationcdhandcrwithcdhr,
    relationcpwithp,
    relationcdhwithcd,
...
onelectureatmostonehourperday:- course(C), day(D), findall(cdh(C,D,H), hour(H), Lits), atMost(1,Lits), fail.
onelectureatmostonehourperday.

twolecturesfromthesameyearnotoverlap:- courseYear(C1,Y), courseYear(C2,Y), C1 < C2, day(D), hour(H), writeClause([-cdh(C1,D,H), -cdh(C2,D,H)]), fail.
twolecturesfromthesameyearnotoverlap.

eachcourseexactlyoneteacher:- courseProfessors(C,LP) , findall(cp(C,P), member(P,LP), Lits), exactly(1,Lits), fail. 
eachcourseexactlyoneteacher.

eachcourseexactlyoneroom:- courseRooms(C,LR), findall(cr(C,R), member(R,LR), Lits), exactly(1,Lits), fail.
eachcourseexactlyoneroom.

eachcourseexactlynumHoursPerWeek:- courseHours(C,Hs), findall(cd(C,D),day(D), Lits), exactly(Hs,Lits), fail.
eachcourseexactlynumHoursPerWeek.


givenateacheranhourandadaycanotbeintwolectures:- courseProfessors(C1,LP), courseProfessors(C2,LP2) C1<C2, member(P,LP), member(P,LP2), day(D), hour(H), writeClause([-cdhp(C1,D,H,P), -cdhp(C2,D,H,P)), fail.
givenateacheranhourandadaycanotbeintwolectures.

givenaroomanhouradaycannotbetwolectures:- courseRooms(C1,LR), courseRooms(C2,LR2) , C1<C2, member(R,LR), member(R,LR2), day(D), hour(H), writeClause([-cdhr(C1,D,H,R), -cdhr(C2,D,H,R)]), fail.
givenaroomanhouradaycannotbetwolectures.

atmostMaxNumProf(MaxNumProf):- findall(p(P), professor(P), Lits), atMost(MaxNumProf,Lits),fail.
atmostMaxNumProf(_).

relationcdhandcpwithcdhp:- courseProfessors(C,LP), member(P,LP), day(D), hour(H), writeClause([-cdh(C,D,H),-cp(C,P),cdhp(C,D,H,P)]), fail.
relationcdhandcpwithcdhp.

relationcdhandcrwithcdhr:- courseRooms(C,LR), member(R,LR), day(D), hour(H), writeClause([-cdh(C,D,H), -cr(C,R), cdhr(C,D,H,R)]), fail.
relationcdhandcrwithcdhr.


relationcpwithp:- courseProfessors(C,LP), member(P,LP), writeClause([-cp(C,P),p(P)]), fail.
relationcpwithp.

relationcdhwithcd:- course(C), day(D),  findall(cdh(C,D,H), hour(H), Lits), expressOr(cd(C,D), Lits), fail.
relationcdhwithcd.
...
costOfThisSolution(M,Cost):- findall(P,member(p(P),M), C1), sort(C1,C2), length(C2,Cost). 
