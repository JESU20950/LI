...
incompatibleWorkers(W1,W2):- workerScore(W1,S1), workerScore(W2,S2), W1 \= W2, maxScore(MaxScore), Stotal is S1+S2, MaxScore<Stotal. 
complexWorkerks(W1,W2):- workerScore(W1,S1), workerScore(W2,S2), W1 \== W2,  S is S1+S2, complexThreshold(Max), S >= Max.
...
writeClauses(MaxComplexTeams):- 
    ...
    atMostMaxComplexTeams(MaxComplexTeams),
    relationCTconWT,
    true,!.
...
eachteamatleastminSize:- team(T), findall(wt(W,T), worker(W), Lits), minSize(S), atLeast(S, Lits), fail.
eachteamatleastminSize.
eachteamatmostmaxSize:- team(T), findall(wt(W,T), worker(W), Lits), maxSize(S), atMost(S, Lits), fail.
eachteamatmostmaxSize.
incompatibleWorkersrestriction:- team(T) , incompatibleWorkers(W1,W2), writeClause([-wt(W1,T), -wt(W2,T)]), fail.
incompatibleWorkersrestriction.

atMostMaxComplexTeams(MaxComplexTeams):- findall(ct(T),  team(T), Lits), atMost(MaxComplexTeams, Lits).



relationCTconWT:- team(T), complexWorkerks(W1,W2), writeClause([-wt(W1,T), -wt(W2,T), ct(T)]), fail.
relationCTconWT.
...
costOfThisSolution(M,Cost):- findall(T, member(ct(T), M) ,C1), sort(C1, C2), length(C2, Cost).
