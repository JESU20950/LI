...
satVariable(bC1C2D1D2(B,C1,C2,D1,D2)):- bus(B), city(C1),city(C2), day(D1), day(D2).

    giventwoconsecutivedayonebuscantravelmorethanmaxDist:- trip(C1-C2), trip(C2-C3), dist(C1,C2,R), dist(C2,C3,R2), Rtotal is R+R2, 
    maxDist(MaxDist), MaxDist < Rtotal, consecutiveDays(D1,D2,D3), bus(B),  writeClause([-bdc(B,D1,C1), -bdc(B,D2,C2), -bdc(B,D3,C3)]), fail.
    giventwoconsecutivedayonebuscantravelmorethanmaxDist.
    
    
    atleastonebusgoesfromonecitytoanother:- trip(C1-C2), findall(bC1C2D1D2(B,C1,C2,D1,D2), (bus(B), consecutiveDays(D1,D2)), Lits), atLeast(1,Lits), fail.
    atleastonebusgoesfromonecitytoanother.
    
    
    relationbdcconbC1C2D1D2:- bus(B), consecutiveDays(D1,D2), trip(C1-C2), writeClause([bC1C2D1D2(B,C1,C2,D1,D2),-bdc(B,D1,C1),-bdc(B,D2,C2)]), fail.
    relationbdcconbC1C2D1D2:- bus(B), consecutiveDays(D1,D2), trip(C1-C2), writeClause([-bC1C2D1D2(B,C1,C2,D1,D2), bdc(B,D1,C1)]), writeClause([-bC1C2D1D2(B,C1,C2,D1,D2), bdc(B,D2,C2)]),fail.
    relationbdcconbC1C2D1D2.
    ...
    onebuseverydayonecity:- bus(B), day(D), findall(bdc(B,D,C), city(C), Lits), exactly(1,Lits),fail.
    onebuseverydayonecity.
    
    givenonedayonecityexactlyonebus:- day(D), city(C), findall(bdc(B,D,C), bus(B), Lits), exactly(1, Lits), fail.
    givenonedayonecityexactlyonebus.
    
    
    everydayonebusgoesfromonecitytoanotherdifferentcity:- consecutiveDays(D1,D2) , bus(B), city(C), writeClause([-bdc(B,D1,C), -bdc(B,D2,C)]), fail.
    everydayonebusgoesfromonecitytoanotherdifferentcity.
